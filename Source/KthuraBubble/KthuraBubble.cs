...
// 
// 
// (c) Jeroen P. Broks, 2019, 2020, 2022
// 
...
namespace KthuraBubble {
	...
	public bool InObj(int id, string objtag,int x, int y) {
		...
		public bool LHasTag(int id,string tag) {
			try {
				if (!Layers.ContainsKey(id)) return false;
				if (!KMaps[id].Layers.ContainsKey(Layers[id])) return false;
		...
		public void SetLayer(int ID,string K) {
			...
			K = K.ToUpper();
			//foreach (var shit in KMaps[ID].Layers) Debug.WriteLine($"Map has layer: {shit.Key} with {shit.Value.Objects.Count} objects");
			Layers[ID] = K;
		}
		...
		public bool ActorExists(int ID,string ActTag) {
			try {
				...
				if (!KMaps.ContainsKey(ID)) { Debug.WriteLine($"{ET}: WARNING! Map non-existent!"); return false; }
				if (!Layers.ContainsKey(ID)) { Debug.WriteLine($"{ET}: WARNING! Layer for map not set!"); return false; }
				if (!KMaps[ID].Layers.ContainsKey(Layers[ID])) { Debug.WriteLine($"{ET}: Referring to non-existent layer '{Layers[ID]}'!"); return false; }
